...
class FundController extends Controller
{
    public function detailFund($id)
    {
        ...
        $deadline = date_diff(date_create(),date_create($fund->deadline." 23:59:59.000000"));
        $persentasefund = explode('.',$fund->dana_masuk/$fund->target*100,2)[0]."%";
        $donaturs = Donatur::where('fund_id',$id)->where('status','Settlement')->orWhere('status','Success')->orderBy('created_at','desc')->paginate(3);
